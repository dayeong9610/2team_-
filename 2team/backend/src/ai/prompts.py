...
def format_messages(messages: list[dict]) -> str:
    # 장면에 포함된 여러 대사를 LLM이 읽기 쉬운 한 문자열로 변환합니다.
    if not messages:
        # 대사가 없는 장면도 프롬프트에 명시할 수 있도록 기본 문구를 반환합니다.
        return "대사 없음"

    lines = []

    for message in messages:
        # 키가 누락된 장면 데이터에도 대응할 수 있도록 기본값을 사용합니다.
        speaker = message.get(
            "speaker",
            "인물"
        )
...
def build_user_prompt(
    ...
    # 현재 Stage의 정보와 사용자 답변을 하나의 평가용 프롬프트로 조합합니다.
